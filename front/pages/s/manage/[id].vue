...
const itemCount = computed(() => data.value?.files?.length ?? 0)
...
<template>
    <BaseCard class="flex flex-col gap-3 my-5" title="分享管理" :showBackButton="true">
        ...
        <div v-else class="flex flex-col gap-3">
            ...
            <h2 class="text-sm font-medium">{{ isFileShare ? t('page.shareManage.files') : t('page.shareManage.textContent') }}</h2>
            <div v-if="isFileShare">
                <ShareFileInfoList v-if="itemCount" :files="data.files || []" />
                <div v-else class="p-5 text-center text-sm text-muted-foreground">
                    {{ t('page.shareManage.noFiles') }}
                </div>
            </div>
            <div
                v-else-if="data.text"
                class="max-h-[55vh] overflow-y-auto rounded-xl border border-black/5 bg-background/60 p-4 dark:border-white/10 sm:p-5"
            >
                <MarkdownRender class="prose w-full max-w-full dark:prose-invert" :markdown="data.text" />
            </div>
            <div v-else class="rounded-xl border border-dashed border-black/10 p-8 text-center text-sm text-muted-foreground dark:border-white/15">
                {{ t('page.shareManage.noText') }}
            </div>
        </div>
    </BaseCard>
</template>
